	...
	filesystem_iterator_entry *entry)
{
	...
	if (!(error = git_buf_joinpath(&fullpath, iter->root, entry->path)) &&
	    !(error = git_path_validate_ondisk_buf(iter->base.repo, &fullpath)))
	...
	filesystem_iterator_entry *frame_entry)
{
	...
	if (git_buf_oom(&root) ||
	    git_path_validate_ondisk_buf(iter->base.repo, &root) < 0) {
		error = -1;
		goto done;
	}
	...
	while ((error = git_path_diriter_next(&diriter)) == 0) {
		...
		if ((error = git_path_diriter_fullpath(&path, &path_len, &diriter)) < 0 ||
		    (error = git_path_validate_ondisk_with_len(iter->base.repo, path, path_len)) < 0)
			goto done;
	...
	const filesystem_iterator_entry *entry)
{
	...
	if ((error = git_buf_joinpath(&fullpath, iter->root, entry->path)) < 0 ||
	    (error = git_path_validate_ondisk_buf(iter->base.repo, &fullpath)) < 0 ||
