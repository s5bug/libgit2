	...
	git_buf **out, checkout_data *data, const char *path)
{
	...
	if (git_path_validate_ondisk_buf(data->repo, &data->target_path) < 0)
		return -1;
	...
	git_merge_file_result *result)
{
	...
	if ((error = git_buf_joinpath(out, data->opts.target_directory, result->path)) < 0 ||
	    (error = git_path_validate_ondisk_buf(data->repo, out)) < 0)
		return error;
...
static int validate_target_directory(checkout_data *data)
{
	...
	if ((error = git_path_validate_ondisk(data->repo, data->opts.target_directory)) < 0)
		return error;
